...
def mergeFromQQMusic(index):
    ...
    dbQQMusic.update({'mid':a['mid']},{'$set':{'inited':True}})
    print 'update Albums '+albumid+' '+str(index)+'/'+str(total)
...
#def initFromNaxosMusicLibrary():
#    total=dbNaxos.find({'inited':{'$exists':False}}).count()
#    albums=dbNaxos.find({'inited':{'$exists':False}})
